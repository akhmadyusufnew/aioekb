...
from bot.forms.states import FormStartPOSReplikasi
from bot.forms.questions import StartPOSReplikasiFormQuestion
...
async def txt_form(state: FSMContext) -> str:
    ...
    string  = f"🍎 #STARTREPLIKASI #{cabang[:4]} #{data_state.get('kdtk', '')}\n"
    string += "<pre><code>"
    string += "Pembuatan POS Replikasi\n"
    string += "--------------------------------------\n\n"
...
@router.message(StateFilter(FormStartPOSReplikasi), F.text.lower() == "/batal")
...
@router.message(FormStartPOSReplikasi.kdtk)
async def input_kdtk(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    try:
        async with get_session() as session_db:
            ...
            await state.set_state(FormStartPOSReplikasi.nomor_co)

            await send_txt_form(bot, session_db, message, state)
            await StartPOSReplikasiFormQuestion(bot, state, session_db).q_nomor_co(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSReplikasi.nomor_co)
    ...
    try:
        async with get_session() as session_db:
            ...
            if text.startswith("/kembali"):
                await state.set_state(FormStartPOSReplikasi.kdtk)
                await StartPOSReplikasiFormQuestion(bot, state, session_db).q_kdtk(message)
                return
            ...
            await state.set_state(FormStartPOSReplikasi.datetime_start)

            await send_txt_form(bot, session_db, message, state)
            await StartPOSReplikasiFormQuestion(bot, state, session_db).q_datetime_start(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSReplikasi.datetime_start)
async def input_datetime_start(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    try:
        async with get_session() as session_db:
            ...
            data_state = await state.get_data()
            id_work = generate_id_work(data_state, "C")

            await state.update_data(id_work=id_work)
            await state.set_state(FormStartPOSReplikasi.alasan)

            await send_txt_form(bot, session_db, message, state)
            await StartPOSReplikasiFormQuestion(bot, state, session_db).q_alasan(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSReplikasi.alasan)
async def input_alasan(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    try:
        async with get_session() as session_db:
            ...
            await state.set_state(FormStartPOSReplikasi.status)
            await StartPOSReplikasiFormQuestion(bot, state, session_db).q_status(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSReplikasi.status)
async def input_status(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    try:
        async with get_session() as session_db:
            ...
            await state.set_state(FormStartPOSReplikasi.keterangan)

            await send_txt_form(bot, session_db, message, state)
            await StartPOSReplikasiFormQuestion(bot, state, session_db).q_keterangan(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSReplikasi.keterangan)
async def input_keterangan(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    try:
        async with get_session() as session_db:
            ...
            await state.set_state(FormStartPOSReplikasi.lampiran_1)

            await send_txt_form(bot, session_db, message, state)
            await StartPOSReplikasiFormQuestion(bot, state, session_db).q_lampiran_1(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSReplikasi.lampiran_1)
async def input_lampiran_1(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    try:
        async with get_session() as session_db:
            ...
            await state.set_state(FormStartPOSReplikasi.lampiran_2)

            await StartPOSReplikasiFormQuestion(bot, state, session_db).q_lampiran_2(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSReplikasi.lampiran_2)
        ...
        async with get_session() as session_db:
            ...
            if message.text:
                ...
                if text.startswith("/selanjutnya"):
                    await state.set_state(FormStartPOSReplikasi.datetime_end)
                    await StartPOSReplikasiFormQuestion(bot, state, session_db).q_datetime_end(message)
                    return
            ...
            await state.update_data(files=files)        
            await state.set_state(FormStartPOSReplikasi.lampiran_3)

            await StartPOSReplikasiFormQuestion(bot, state, session_db).q_lampiran_3(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSReplikasi.lampiran_3)
async def input_lampiran_3(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    try:
        async with get_session() as session_db:
            ...
            await state.update_data(files=files)        
            await state.set_state(FormStartPOSReplikasi.lampiran_4)

            await StartPOSReplikasiFormQuestion(bot, state, session_db).q_lampiran_4(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSReplikasi.lampiran_4)
...
@router.message(FormStartPOSReplikasi.datetime_end)
        ...
        async with get_session() as session_db:
            ...
            if text.startswith("/kembali"):
                ...
                for num in (4, 3, 2, 1):
                    ...
                    await state.set_state(getattr(FormStartPOSReplikasi, f"lampiran_{num}"))
                    await getattr(
                        StartPOSReplikasiFormQuestion(bot, state, session_db),
                        f"q_lampiran_{num}"
                    )(message)
            ...
            await state.set_state(FormStartPOSReplikasi.simpan)
            await send_txt_form(bot, session_db, message, state)
            await StartPOSReplikasiFormQuestion(bot, state, session_db).q_kirim(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSReplikasi.simpan)
