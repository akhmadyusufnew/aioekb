...
async def txt_form(state: FSMContext) -> str:
    ...
    string  = f"🍋 #TUTUPSIAGA #{cabang[:4]} #{data_state.get('kdtk', '')}\n"
    string += "<pre><code>"
    string += "Penutupan POS Siaga\n"
    string += "------------------------------------\n\n"
...
@router.message(StateFilter(FormTutupPOSSiaga), F.text.lower() == "/batal")
...
@router.message(FormTutupPOSSiaga.kdtk)
...
@router.message(FormTutupPOSSiaga.nomor_co)
...
@router.message(FormTutupPOSSiaga.station)
...
@router.message(FormTutupPOSSiaga.status)
...
@router.message(FormTutupPOSSiaga.keterangan)
...
@router.message(FormTutupPOSSiaga.lampiran_1)
...
@router.message(FormTutupPOSSiaga.lampiran_2)
...
@router.message(FormTutupPOSSiaga.lampiran_3)
...
@router.message(FormTutupPOSSiaga.lampiran_4)
...
@router.message(FormTutupPOSSiaga.datetime_end)
async def input_datetime_end(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    try:
        async with get_session() as session_db:
            ...
            data_state = await state.get_data()
            id_work = generate_id_work(data_state, "F")
...
@router.message(FormTutupPOSSiaga.simpan)
