...
from bot.forms.states import FormStartPOSSiaga
from bot.forms.questions import StartPOSSiagaFormQuestion
...
async def txt_form(state: FSMContext) -> str:
    ...
    string  = f"🍊 #STARTSIAGA #{cabang[:4]} #{data_state.get('kdtk', '')}\n"
    string += "<pre><code>"
    string += "Pembuatan POS Siaga\n"
    string += "------------------------------------\n\n"
    ...
    string += "Time Mulai".ljust(12) + f": {datetime_start}\n"
    string += "Time Selesai".ljust(12) + f": {datetime_end}\n"
    string += "Station".ljust(12) + f": {data_state.get('station', '-')}\n"
    string += "Alasan".ljust(12) + f": {data_state.get('alasan', '-')}\n"
    string += "? Tdk Rplksi".ljust(12) + f": {data_state.get('alasan_2', '-')}\n"
    string += "\n"
...
@router.message(StateFilter(FormStartPOSSiaga), F.text.lower() == "/batal")
...
@router.message(FormStartPOSSiaga.kdtk)
async def input_kdtk(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    try:
        async with get_session() as session_db:
            ...
            await state.set_state(FormStartPOSSiaga.nomor_co)

            await send_txt_form(bot, session_db, message, state)
            await StartPOSSiagaFormQuestion(bot, state, session_db).q_nomor_co(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSSiaga.nomor_co)
    ...
    try:
        async with get_session() as session_db:
            ...
            if text.startswith("/kembali"):
                await state.set_state(FormStartPOSSiaga.kdtk)
                await StartPOSSiagaFormQuestion(bot, state, session_db).q_kdtk(message)
                return
            ...
            await state.set_state(FormStartPOSSiaga.station)

            await send_txt_form(bot, session_db, message, state)
            await StartPOSSiagaFormQuestion(bot, state, session_db).q_station(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSSiaga.station)
async def input_station(message: Message, bot: Bot, state: FSMContext) -> None:
    await safe_delete_message(message)
    ...
    try:
        async with get_session() as session_db:
            ...
            await state.set_state(FormStartPOSSiaga.datetime_start)

            await send_txt_form(bot, session_db, message, state)
            await StartPOSSiagaFormQuestion(bot, state, session_db).q_datetime_start(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSSiaga.datetime_start)
async def input_datetime_start(message: Message, bot: Bot, state: FSMContext) -> None:
    await safe_delete_message(message)
    ...
    try:
        async with get_session() as session_db:
            ...
            data_state = await state.get_data()
            id_work = generate_id_work(data_state, "E")

            await state.update_data(id_work=id_work)
            await state.set_state(FormStartPOSSiaga.alasan)

            await send_txt_form(bot, session_db, message, state)
            await StartPOSSiagaFormQuestion(bot, state, session_db).q_alasan(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSSiaga.alasan)
async def input_alasan(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    try:
        async with get_session() as session_db:
            ...
            await state.update_data(alasan=raw_text)
            await send_txt_form(bot, session_db, message, state)

            if any(k in text for k in ("backup", "restore", "database")):
                await state.set_state(FormStartPOSSiaga.status)
                await StartPOSSiagaFormQuestion(bot, state, session_db).q_status(message)
            else:
                await state.set_state(FormStartPOSSiaga.alasan_2)
                await StartPOSSiagaFormQuestion(bot, state, session_db).q_alasan_2(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSSiaga.alasan_2)
async def input_alasan_2(message: Message, bot: Bot, state: FSMContext) -> None:
    await safe_delete_message(message)
    ...
    try:
        async with get_session() as session_db:
            ...
            await state.update_data(alasan_2=raw_text)
            await state.set_state(FormStartPOSSiaga.status)
...
@router.message(FormStartPOSSiaga.status)
    ...
    try:
        async with get_session() as session_db:
            ...
            if text.startswith("/kembali"):
                ...
                if data_state.get("alasan_2"):
                    next_state = FormStartPOSSiaga.alasan_2
                    next_question = StartPOSSiagaFormQuestion(bot, state, session_db).q_alasan_2
                else:
                    next_state = FormStartPOSSiaga.alasan
                    next_question = StartPOSSiagaFormQuestion(bot, state, session_db).q_alasan
            ...
            await state.set_state(FormStartPOSSiaga.keterangan)

            await send_txt_form(bot, session_db, message, state)
            await StartPOSSiagaFormQuestion(bot, state, session_db).q_keterangan(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSSiaga.keterangan)
async def input_keterangan(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    try:
        async with get_session() as session_db:
            ...
            await state.set_state(FormStartPOSSiaga.lampiran_1)

            await send_txt_form(bot, session_db, message, state)
            await StartPOSSiagaFormQuestion(bot, state, session_db).q_lampiran_1(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSSiaga.lampiran_1)
async def input_lampiran_1(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    try:
        async with get_session() as session_db:
            ...
            await state.set_state(FormStartPOSSiaga.lampiran_2)

            await StartPOSSiagaFormQuestion(bot, state, session_db).q_lampiran_2(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSSiaga.lampiran_2)
        ...
        async with get_session() as session_db:
            ...
            if message.text:
                ...
                if text.startswith("/selanjutnya"):
                    await state.set_state(FormStartPOSSiaga.datetime_end)
                    await StartPOSSiagaFormQuestion(bot, state, session_db).q_datetime_end(message)
                    return
            ...
            await state.update_data(files=files)        
            await state.set_state(FormStartPOSSiaga.lampiran_3)

            await StartPOSSiagaFormQuestion(bot, state, session_db).q_lampiran_3(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSSiaga.lampiran_3)
async def input_lampiran_3(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    try:
        async with get_session() as session_db:
            ...
            await state.update_data(files=files)        
            await state.set_state(FormStartPOSSiaga.lampiran_4)

            await StartPOSSiagaFormQuestion(bot, state, session_db).q_lampiran_4(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSSiaga.lampiran_4)
...
@router.message(FormStartPOSSiaga.datetime_end)
        ...
        async with get_session() as session_db:
            ...
            if text.startswith("/kembali"):
                ...
                for num in (4, 3, 2, 1):
                    ...
                    await state.set_state(getattr(FormStartPOSSiaga, f"lampiran_{num}"))
                    await getattr(
                        StartPOSSiagaFormQuestion(bot, state, session_db),
                        f"q_lampiran_{num}"
                    )(message)
            ...
            await state.set_state(FormStartPOSSiaga.simpan)
            await send_txt_form(bot, session_db, message, state)
            await StartPOSSiagaFormQuestion(bot, state, session_db).q_kirim(message)

    except Exception as e:
        await handle_exception(message, bot, e)


@router.message(FormStartPOSSiaga.simpan)
        ...
        async with get_session() as session_db:
            ...
            session_db.add(
                SiagadanReplikasiDB(
                    ...
                    alasan2=data_state.get("alasan_2"),
